...
def test_skills_install_is_rootless_and_omits_heavy_surface(tmp_path: Path) -> None:
    ...
    assert (paths.bin / "rsx").is_file()
    assert "listen|send|read|status|stop" in (paths.bin / "rsx").read_text()
    ...
    assert (paths.home / ".codex/skills/reverse-shell-async").is_symlink()
    assert (paths.home / ".config/opencode/skills/reverse-shell-async").is_symlink()
