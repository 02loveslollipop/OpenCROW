...
class RuntimeControlWebSocket(tornado.websocket.WebSocketHandler):
    ...
    def on_message(self, message: str) -> None:
        try:
            ...
            return
        if not isinstance(payload, dict):
            self.write_message(json.dumps({"event_type": "error", "error": "JSON payload must be an object"}))
            return
        ...
        try:
            ...
            import logging
            logging.error("Unhandled exception in RuntimeHandler.on_message", exc_info=True)
...
class ConstellationWebSocket(tornado.websocket.WebSocketHandler):
    ...
    def _watch_events(self) -> None:
        try:
            ...
            import logging
            logging.error("Unhandled exception in ChatTopicHandler._watch_events", exc_info=True)
            if self.io_loop is not None:
