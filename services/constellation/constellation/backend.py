...
class RuntimeControlWebSocket(tornado.websocket.WebSocketHandler):
    ...
    def on_message(self, message: str) -> None:
        try:
            ...
            return
        if not isinstance(payload, dict):
            self.write_message(json.dumps({"event_type": "error", "error": "Invalid payload format"}))
            return
        ...
        try:
            ...
            logging.error("Internal server error in RuntimeControlWebSocket", exc_info=True)
...
class ConstellationWebSocket(tornado.websocket.WebSocketHandler):
    ...
    def _watch_events(self) -> None:
        try:
            ...
            logging.error("Internal server error in _watch_events", exc_info=True)
            if self.io_loop is not None:
