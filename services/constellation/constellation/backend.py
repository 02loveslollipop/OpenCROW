...
class RuntimeControlWebSocket(tornado.websocket.WebSocketHandler):
    ...
    def on_message(self, message: str) -> None:
        try:
            ...
            return
        if not isinstance(payload, dict):
            self.write_message(json.dumps({"event_type": "error", "error": "Invalid JSON payload: expected a dictionary"}))
            return
...
class ConstellationWebSocket(tornado.websocket.WebSocketHandler):
    ...
    def _watch_events(self) -> None:
        try:
            ...
            import logging
            logging.error("Unhandled exception in ConstellationWebSocket event watcher", exc_info=True)
            if self.io_loop is not None:
