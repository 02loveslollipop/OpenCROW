...
OPENCROW_TARGET_USER=opencrow-ci bash "$ROOT_DIR/installer/install.sh" \
  --agents codex --toolboxes utility --no-miniconda --yes
...
sudo -H -u opencrow-skills env \
  ...
  bash "$ROOT_DIR/installer/skills.sh" --agents codex --yes
